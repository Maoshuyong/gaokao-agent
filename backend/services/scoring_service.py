...
class ScoringService:
    ...
    # 档位阈值（rank_ratio 区间）- P0修复：放宽阈值
    # 位次越小越好：ratio < 1 表示考生位次更高（更好）
    LEVEL_THRESHOLDS = {
        "冲刺":  (0.70, 1.15),   # 修复：从(0.80, 1.05) 放宽到 (0.70, 1.15)
        "稳妥":  (0.40, 0.70),   # 修复：从(0.55, 0.80) 调整到 (0.40, 0.70)
        "保底":  (0.01, 0.40),   # 修复：从(0.01, 0.55) 调整到 (0.01, 0.40)
        "不建议": (1.15, 999.0),  # 修复：从(1.05, 999.0) 放宽到 (1.15, 999.0)
    }
    ...
    def calculate_avg_rank(self, scores: List[Score]) -> Optional[float]:
        ...
        if len(valid_ranks) > 1:
            # P0修复：降低最差年份权重，从0.6降到0.4
            avg = sum(valid_ranks) / len(valid_ranks)
            return avg * 0.6 + worst_rank * 0.4  # 修复：降低保守程度
        return worst_rank
